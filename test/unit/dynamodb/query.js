...
const Query = require('../../../lib/dynamodb/query');

test('must override stopOnEmpty and flush', t => {
  t.plan(2);

  const stream = new Query({}, { stopOnEmpty: true, flush: false });
...
test('getRecords - passes through params', t => {
  ...
  const stream = new Query({ TableName: 'foo', ProjectionExpression: '#foo, #bar', });
...
test('getRecords - tracks LastEvaluatedKey', t => {
  ...
  const stream = new Query({ TableName: 'foo' });
